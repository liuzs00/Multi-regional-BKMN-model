...
| §2.11 Okun `ΔU = κΔY`, β_Phillips = 0 | `oprisk` | sign chain; input is Ω only (§20c); stays inside the reference's saturating bound (§20d) |
...
| 17 | Eq 15 `Δr(t) = ∫Δr^Policy ds` | Taylor output at *t* used directly | **RESOLVED — no longer a deviation.** The reference defines `rates.short_rate_shift` (the eq-15 integral) but its pipeline never calls it: it passes `policy_shift(...)` straight into `zc_shift`. We do the same. Verified by grep — 0 occurrences of `short_rate_shift` in `pipeline.py`. |
...
| 20 | Prop 1 direct **and** cascading physical effect | direct only | [ambiguous] applying both double-counts — now **quantified**. Direct (`−VL·α`) reproduces Ω exactly by the Prop-1 identity: world GVA-weighted −0.7795 % against Ω(1.5 °C) = −0.7795 %. The cascade (`ct += VL·α` through the dual) gives 0.42× that at φ=0.5, so applying both would count the damage **1.42×**. The reference uses the cascade for the CDS/equity channel *only*, and passes Ω directly to the Taylor rule and op-risk — which is what we do. |
| 20b | Prop 1 cascade in the **market** channel | our equity is `β·ΔGVA` at region level, with no dual propagation of the damage | [deviation] the reference propagates `VL·α` through the Leontief dual before hitting the equity beta. Affects the equity channel only; our region-level aggregation makes the sector-weighting question moot, but the propagation is genuinely absent. |
| 20c | §2.11 op-risk input | **CORRECTED.** We fed transition + physical + tariff to Okun's law; the reference calls `oprisk.shift(omega, ...)` and never passes it the carbon shock. Okun maps real **output** to employment, and a tax wedge destroys no output — the same argument that removed the transition shock from the Taylor rule (16b), which we had applied to one and not the other. Now physical-only; `OPRISK_INPUT` in run_fx.py. Conduct at 2040 Net Zero falls from **2.0–37.1 %** to **2.0–10.2 %**. |
| 20d | Eq 25 op-risk **functional form** | reference: `m·(−κ)·Ω/(offset+Ω)`, saturating, bounded by `m·(−κ)` (Conduct 23.77 %, Execution 28.52 %). Ours: `β₁·(κ·ΔY)/U₀`, the linearisation of the same Table-10 log-log regression, **unbounded** | [deviation] ours uses a per-region base unemployment rate (2.83 % RASIA to 10.46 % TUR) where the reference uses one fixed offset of 0.041; that is an improvement for a multi-region model, but the linear form can exceed a ceiling the reference cannot — it did, at 37.1 %, before 20c. A gate now gates the bound. |
| 20e | Ω at the **valuation date** vs at the **horizon** | reference: `omega_t0` drives the Taylor rule and op-risk, `omega_hz` drives the Prop-1 cascade. Ours: Ω(horizon) everywhere | [framing] the reference prices "what if today's damage repriced now" at a series of tenors, so its climate rate shift is *constant* across horizons and all variation comes from the market baseline we do not model. Ours is a 2025–2045 projection, where a horizon-invariant rate shift would be wrong. Recorded because it means our rate path is not directly comparable to the paper's Table 12. |
...
| 23b | §2.6 the currency the Moessner coefficient is applied to | paper writes a **dollar** input, but its printed inflation row is reproduced only by applying `k` to the **sterling** price (`8e-5 × £11.45 → 9.2 bp` vs printed 9; the \$15.36 price gives 12.3). The single-region reproduction establishes this at every horizon. **We apply `k` to USD**, this model's numéraire. | [unresolved, and the largest open item] If the paper's reading is right, our inflation channel — and with it **every spot FX move** — is overstated by the GBP/USD rate, **1.341×**. USD/EUR at 2040 under Net Zero would go −1.95 % → −1.46 %. Note the factor does **not** cancel in spot: a difference of two cumulative inflations both scaled by `c` is scaled by `c`, not restored. What *is* invariant is the ratio between currencies and the ranking — so §2 of FX_REPORT survives, §3's levels do not. Settling it needs Moessner's own units, which we have not verified. |
| 23c | §3.3 tail construction | paper: two independent Dirichlets over SSP and RCP, whose lost mass on infeasible pairs generates the 99.9 % tail. Ours: **z·σ input stress** | [multi-R, and the mechanism cannot transfer] The NGFS set is one-dimensional with no feasibility matrix, so weights sum to 1 and no mass can be lost. Worth noting the reference *tested and rejected* a σ-stress for the paper's setting — Table 17's σ of 0.4–1.2 °C against a reported 1-year temperature *change* of 0.0223 °C swamps it ~30×. That objection does not transfer: we stress the temperature **level** (σ = 0.27 K on 1.65 K at 2040, i.e. 1.64σ ≈ 27 % of the driver), so the stress is a fraction of the driver rather than a multiple of it. The level convention adopted for the damage function is what makes it well posed. |
